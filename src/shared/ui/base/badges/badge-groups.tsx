...
import { cn, sortCn } from '@/shared/lib/utils'
...
const getSizeClasses = (
  ...
  leading: {
    md: {
      root: cn('py-1 pr-2 pl-1 text-xs font-medium', !text && !icon && 'pr-1'),
      addon: cn('px-2 py-0.5', theme === 'modern' && 'gap-1 px-1.5', text && 'mr-2'),
      icon: 'ml-1 size-4',
    },
    lg: {
      root: cn('py-1 pr-2 pl-1 text-sm font-medium', !text && !icon && 'pr-1'),
      addon: cn('px-2.5 py-0.5', theme === 'modern' && 'gap-1.5 px-2', text && 'mr-2'),
      icon: 'ml-1 size-4',
    },
  },
  trailing: {
    md: {
      root: cn('py-1 pr-1 pl-3 text-xs font-medium', theme === 'modern' && 'pl-2.5'),
      addon: cn('py-0.5 pr-1.5 pl-2', theme === 'modern' && 'pr-1.5 pl-2', text && 'ml-2'),
      icon: 'ml-0.5 size-3 stroke-[3px]',
      dot: 'mr-1.5',
    },
    lg: {
      root: 'py-1 pr-1 pl-3 text-sm font-medium',
      addon: cn('py-0.5 pr-2 pl-2.5', theme === 'modern' && 'pr-1.5 pl-2', text && 'ml-2'),
...
export const BadgeGroup = ({
  ...
  const dotClasses = cn('inline-block size-2 shrink-0 rounded-full', sizes.dot, colors.dot)
  const iconClasses = cn(baseClasses[theme].icon, sizes.icon, colors.icon)
