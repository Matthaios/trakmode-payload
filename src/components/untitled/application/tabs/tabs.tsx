...
import type { BadgeColors } from '@/components/untitled/base/badges/badge-types'
...
const getHorizontalStyles = ({ size, fullWidth }: { size?: 'sm' | 'md'; fullWidth?: boolean }) => ({
  ...
  underline: cn('gap-3', fullWidth && 'w-full gap-4'),
  line: 'gap-2',
})
        ...
        <Fragment>
          ...
          {badge && (
            <Badge
              ...
              className={cn('hidden transition-inherit-all md:flex', size === 'sm' && '-my-px')}
            >
              {badge}
            </Badge>
          )}
