...
export const LoadingIndicator = ({
  ...
  const renderSpinner = () => {
    if (type === 'line-spinner') {
      return (
        <svg className={cn('animate-spin', styles[size].spinner)} viewBox="0 0 32 32" fill="none">
          <circle
    ...
    if (type === 'dot-circle') {
      return (
        <svg
          className={cn('animate-spin text-fg-brand-primary', styles[size].spinner)}
          viewBox="0 0 36 36"
          fill="none"
        >
          <path
  ...
  return (
    <div className={cn('flex flex-col items-center justify-center', styles[size].root)}>
      {renderSpinner()}
      {label && <span className={cn('text-secondary', styles[size].label)}>{label}</span>}
    </div>
  )
}
