...
import { CloseButton } from '@/components/untitled/base/buttons/close-button'
...
const Menu = ({ children, dialogClassName, ...props }: SlideoutMenuProps) => {
  return (
    <ModalOverlay {...props}>
      <Modal
        className={(state) =>
          cn(typeof props.className === 'function' ? props.className(state) : props.className)
        }
      >
        {(state) => (
...
const Header = ({ className, children, onClose, ...props }: SlideoutHeaderProps) => {
  return (
    <header {...props} className={cn('relative z-1 w-full px-4 pt-6 md:px-6', className)}>
      {children}
